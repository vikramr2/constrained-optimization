...
def par_task(stack, node_mapping, node2cids):
    ...
    while stack:
        ...
        # Cluster Mode
            ...
            subp = list(clusterer.cluster_without_singletons(subgraph))
            subp = [s.realize(subgraph) for s in subp]

            # Exit condition: the clustering algo did nothing
            if len(subp) == 1 and subp[0].n() == subgraph.n():
                if not quiet_g:
                    log.info("clustering did nothing, ending algo on this cluster")
            else:
                tree_node.cm_valid = False
                tree_node.extant = False
